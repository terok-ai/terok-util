...
"""Tests for the centralised YAML I/O facade (ruamel.yaml round-trip)."""
...
from terok_util.yaml import YAMLError, dump, load, yaml_dump, yaml_load
...
class TestThreadSafety:
    ...
    'anchor'`` deep inside the Textual TUI's shield-state worker.  The
    facade hands out a fresh instance per call to stay re-entrant.
    """
...
class TestAliases:
    """``yaml_load`` / ``yaml_dump`` are flat aliases of ``load`` / ``dump``."""

    def test_yaml_load_is_load(self) -> None:
        assert yaml_load is load

    def test_yaml_dump_is_dump(self) -> None:
        assert yaml_dump is dump
