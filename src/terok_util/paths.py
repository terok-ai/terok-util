...
def host_uid() -> int:
    ...
    *outer* (host) UID via the userns ``uid_map`` translation; if the
    process advertises the inner UID over the wire the peer rejects
    it as a credential mismatch.

    This helper translates ``geteuid()`` through ``/proc/self/uid_map``:
    finds the row whose inner range covers the effective UID and
    returns the corresponding outer UID.  When ``/proc`` is unavailable
    (macOS, BSD, exotic chroot) or no row matches, falls back to the
    bare ``geteuid()`` answer — correct on systems without Linux user
    namespaces.
    """
    try:
        euid = os.geteuid()
